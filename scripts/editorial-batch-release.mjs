...
export function prepareBatchRelease({
  root,
  items,
}) {
  ...
  for (const item of items) {
    allowedPaths.add(
      normalizePath(
        relative(root, item.articlePath),
      ),
    );
  ...
  if (unexpectedPaths.length > 0) {
    throw new Error(
      'existem alterações fora do lote editorial:\n' +
  ...
  for (const item of items) {
    ...
    if (!changedPaths.includes(articleRelativePath)) {
      throw new Error(
        `o artigo não possui alterações para publicar: ${item.slug}`,
      );
    }
  }
}

async function waitForPublishedArticle({
  section,
  slug,
  title,
  commit,
  ...
  for (
    ...
    if (attempt < attempts) {
      console.log(
        `Deploy ainda não confirmado para ${slug} (${attempt}/${attempts}).`,
      );
...
export async function releaseBatch({
  ...
  run(
    ...
    [
      'commit',
      '-m',
      `feat: publish editorial batch ${date}`,
    ],
    root,
  );
